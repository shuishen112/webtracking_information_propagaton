...
class Dataloader:
    ...
    def _get_iter(self, start_idx=0):
        ...
        for i in range(start_idx, len(self.ds)):
            ...
            adj = biadjacency_to_laplacian(B) * self.alpha #[n_users+n_items, n_users+n_items]
            i2u_adj, u2i_adj = biadjacency_to_propagation(delta_B) #[n_users, n_items], [n_items, n_users]
...
class Dataset:
    ...
    def process_timestamps(self, ts):
        """
        Given a list of timestamps, returns a tuple of two arrays:
        - unique_ts: an array of unique timestamps
        - cum_n_records: an array of cumulative number of records up to each unique timestamp
        """
        unique_ts = np.unique(ts)
        # for each unique timestamp, find the index record with that timestamp
        end_idx_ts_dict = {t: i+1 for i, t in enumerate(ts)}
        # for the unique timestamp, find the index of the last record with that timestamp
    ...
    def build_ui_mat(self, df):
        row = df.iloc[:, 0]
        col = df.iloc[:, 1]
        data = np.ones(len(df)) # 1686 users, 962 items
